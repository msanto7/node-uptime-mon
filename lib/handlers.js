...
handlers.index = function(data, callback) {
	...
	if(data.method == 'get') {
		...
		var templateData = {
			...
			'head.description' : 'Uptime monitoring for HTTP and HTTPS sites. Text messages sent when a site goes down. ',
			'body.class' : 'index'
		};
...
handlers.accountCreate = function(data, callback) {
	...
	if(data.method == 'get') {
		...
		var templateData = {
			...
			'head.description' : 'Sign up only takes a few seconds',
...
handlers.sessionCreate = function(data, callback) {
	...
	if(data.method == 'get') {
		...
		var templateData = {
			...
			'head.description' : 'Please enter your phone number and password to access your acount.',
...
handlers.favicon = function(data, callback) {
	// Reject any method that isnt a GET
	if(data.method == 'get') {
										...
										_data.delete('checks', checkId, function(err) {
											...
											if(checksDeleted == checksToDelete) {
												if (!deletionErrors) {
													...
													callback(500, {'Error' : 'Erros encountered while attempting to delete all of the users checks'});
												}
											}
										});
									});
	...
	if (id) {
		...
		_data.read('checks', id, function(err, checkData) {
			if(!err && checkData) {
				// get the token from the headers
				var token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
				//verify that the given token is valid and belongs to the user creating the check
...
handlers._checks.put = function(data, callback) {
	...
	if(id) {
		...
		//check to makesure 1 optional field is filled in
		if (protocol || url || method || successCodes || timeoutSeconds) {
			// look up the check
			_data.read('checks', id, function(err, checkData) {
				if(!err && checkData) {
	...
	if (id) { 
		...
		_data.read('checks', id, function(err, checkData) {
			if(!err && checkData) {
				...
				callback(400, {'Error' : 'The specified check ID does not exits.'});
			}
		});	
	} else {
			callback(400, {'Error' : 'Missing required field'});
